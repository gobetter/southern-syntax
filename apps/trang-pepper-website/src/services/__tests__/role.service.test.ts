...
describe('Role Service', () => {
  ...
  const superAdminActor = { id: 'superadmin-user-id', role: { key: ROLE_NAMES.SUPERADMIN } };
  const adminActor: Partial<UserWithRole> = { id: 'admin-user-id', role: { key: 'ADMIN' } };
  ...
  describe('updateRole', () => {
    ...
    it('should FORBID an Admin from updating a system role', async () => {
      ...
      await expect(roleService.updateRole(systemRole.id, input, adminActor.id!)).rejects.toThrow(
  ...
  describe('deleteRole', () => {
    it('should FORBID an Admin from deleting a system role', async () => {
      ...
      await expect(roleService.deleteRole(systemRole.id, adminActor.id!)).rejects.toThrow(
    ...
    it('should prevent deletion of a role with assigned users', async () => {
      ...
      await expect(roleService.deleteRole(customRole.id, superAdminActor.id!)).rejects.toThrow(
        'ROLE_IN_USE',
      );
    });
  });
