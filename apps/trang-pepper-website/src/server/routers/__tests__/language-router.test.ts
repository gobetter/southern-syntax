...
describe('languageRouter', () => {
  ...
  it('getAll calls service', async () => {
    const spy = vi.spyOn(languageService, 'getAllLanguages').mockResolvedValue([]);
  ...
  it('create calls service', async () => {
    const spy = vi.spyOn(languageService, 'createLanguage').mockResolvedValue(mockLanguage);
    const session: Session = { user: { id: 'u1' }, expires: '' };
    const caller = languageRouter.createCaller({ session, prisma: {} as unknown as PrismaClient });
    const input = { code: 'en', name: 'English', isDefault: false, isActive: true };
