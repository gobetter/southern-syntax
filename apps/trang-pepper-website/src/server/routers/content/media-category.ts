...
import { mediaCategoryInputSchema } from '@/schemas/media-taxonomy';

export const mediaCategoryRouter = router({
  getAll: authorizedProcedure(PERMISSION_RESOURCES.MEDIA_TAXONOMY, PERMISSION_ACTIONS.READ).query(
    async () => {
      return mediaCategoryService.getAll();
    },
  ),
  create: authorizedProcedure(PERMISSION_RESOURCES.MEDIA_TAXONOMY, PERMISSION_ACTIONS.CREATE)
  ...
  update: authorizedProcedure(PERMISSION_RESOURCES.MEDIA_TAXONOMY, PERMISSION_ACTIONS.UPDATE)
  ...
  delete: authorizedProcedure(PERMISSION_RESOURCES.MEDIA_TAXONOMY, PERMISSION_ACTIONS.DELETE)
    .input(idParamSchema)
    .mutation(({ input, ctx }) => {
      const actorId = ctx.session.user.id;
