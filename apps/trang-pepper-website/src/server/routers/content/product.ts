...
export const productRouter = router({
  ...
  // สร้างสินค้าใหม่
  create: authorizedProcedure(PERMISSION_RESOURCES.PRODUCT, PERMISSION_ACTIONS.CREATE) // <-- เปลี่ยนจาก protectedProcedure เป็น authorizedProcedure
  ...
  // อัปเดตข้อมูลสินค้า
  update: authorizedProcedure(PERMISSION_RESOURCES.PRODUCT, PERMISSION_ACTIONS.UPDATE)
    .input(
      z.object({
  ...
  // ลบสินค้า
  delete: authorizedProcedure(PERMISSION_RESOURCES.PRODUCT, PERMISSION_ACTIONS.DELETE)
