...
import { Category } from '@/types/media-taxonomy';
...
export function useCategoryManager({ formMethods }: UseCategoryManagerProps) {
  ...
  const { data: categoriesData, isLoading } = trpc.mediaCategory.getAll.useQuery();
  ...
  const categories = useMemo<Category[]>(
    () =>
      mapIdSlugName(
        categoriesData as Array<{ id: string; slug: string; name: LocalizedString }> | undefined,
      ) as Category[],
    [categoriesData],
  );
