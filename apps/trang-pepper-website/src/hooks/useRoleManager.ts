...
export function useRoleManager() {
  ...
  const { data: roles, isLoading: isLoadingRoles, isError, error } = trpc.role.getAll.useQuery();
  const { data: permissions, isLoading: isLoadingPermissions } = trpc.permission.getAll.useQuery();
  ...
  const handleEdit = (role: Role) => {
    reset({
      ...
      permissionIds: role.permissions.map((p: { permissionId: string }) => p.permissionId),
    });
    setEditingRole(role);
    setDialogOpen(true);
  };
